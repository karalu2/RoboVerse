...
class TestServer:
    ...
    def test_multiple_clients(self, Server, addr):
        ...
        with server:
            ...
            futures = [client.function({"foo": i}) for i, client in enumerate(clients)]
    ...
    def test_maxinflight(self, Server, addr):
        ...
        with server:
            client = embodied.distr.Client(addr, 0, pings=0, maxage=1, maxinflight=2)
            client.connect(retry=False, timeout=1)
    ...
    def test_proxy_batched(self, Server, inner_addr, outer_addr, workers):
        ...
        proxy_server.bind(
            "function",
            lambda x: proxy_client.function(x).result(),
            batch=2,
            workers=workers,
        )
        server = Server(inner_addr)
        server.bind("function", lambda data: {"foo": 2 * data["foo"]}, workers=workers)
        with server:
            proxy_client.connect(retry=False, timeout=1)
            with proxy_server:
                client = embodied.distr.Client(outer_addr, pings=0, maxage=1)
