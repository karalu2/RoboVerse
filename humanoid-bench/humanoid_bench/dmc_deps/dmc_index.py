...
import humanoid_bench.dmc_deps.dmc_sizes as sizes
import humanoid_bench.dmc_deps.dmc_util as util
import numpy as np
...
def make_axis_indexers(model):
    ...
    for size_name in size_name_to_element_names:
        ...
        if size_name in _RAGGED_ADDRS:
            ...
            indexer = RaggedNamedAxis(element_names, element_sizes, singleton=singleton)
        else:
...
class RegularNamedAxis(Axis):
    ...
    def __init__(self, names):
        ...
        """
        self._names = names
        self._names_to_offsets = {name: offset for offset, name in enumerate(names) if name}
    ...
    def convert_key_item(self, key_item):
        ...
        if isinstance(key_item, str):
            ...
            if isinstance(key_item.flat[0], str):
                key_item = np.array([self._names_to_offsets[util.to_native_string(k)] for k in key_item.flat])
...
class FieldIndexer:
    ...
    __slots__ = ("_axes", "_field", "_field_name")
    ...
    def _convert_key(self, key):
        ...
        if len(key) > self._field.ndim:
            raise IndexError(f"Index tuple has {len(key)} elements, but array has only {self._field.ndim} dimensions.")
        new_key = tuple(axis.convert_key_item(key_item) for axis, key_item in zip(self._axes, key))
        if not return_tuple:
            new_key = new_key[0]
        return new_key
    ...
    def __repr__(self):
        ...
        lines.append(cls_template.format(class_name=self.__class__.__name__, field_name=self._field_name))
        ...
        if col_name_len:
            ...
            col_names = "".join(
                "{name:<{width:}}".format(name=util.to_native_string(name), width=col_width) for name in col_name_arr
            )
        ...
        if not self._field.shape[0]:
            ...
            for idx, row in enumerate(self._field):
                row_vals = np.array2string(
                    np.atleast_1d(row), suppress_small=True, formatter={"float_kind": "{: < 9.3g}".format}
                )
                lines.append(
                    row_template.format(
                        idx=idx,
...
def struct_indexer(struct, struct_name, size_to_axis_indexer):
    ...
    for field_name in array_sizes:
        ...
        field_indexers[field_name] = FieldIndexer(
            parent_struct=struct, field_name=field_name, axis_indexers=axis_indexers
        )
